...
function obj:_processApiResponse(body)
    ...
    local copyright = imageData.copyright or ""
    local copyrightlink = imageData.copyrightlink or ""
    ...
    self:_downloadWallpaperImage(imageUrl, fallbackUrl, title, hash, copyright, copyrightlink)
end

function obj:_downloadWallpaperImage(imageUrl, fallbackUrl, title, hash, copyright, copyrightlink)
    ...
    if hs.fs.attributes(filepath) then
        ...
        self:_saveWallpaperInfo(title, copyright, copyrightlink, hash, filepath)
        return
    end
    ...
    hs.http.asyncGet(imageUrl, nil, function(status, body, headers)
        if status == 200 then
            self:_saveWallpaper(body, filepath, title, false, copyright, copyrightlink, hash)
        else
            ...
            hs.http.asyncGet(fallbackUrl, nil, function(status2, body2, headers2)
                if status2 == 200 then
                    self:_saveWallpaper(body2, filepath, title, false, copyright, copyrightlink, hash)
                else
...
function obj:_saveWallpaper(imageData, filepath, title, wasOverwrite, copyright, copyrightlink, hash)
    ...
    if file then
        ...
        -- Save wallpaper information to file
        self:_saveWallpaperInfo(title, copyright, copyrightlink, hash, filepath)
...
function obj:_saveWallpaperInfo(title, copyright, copyrightlink, hash, filepath)
    local infoFile = self.wallpaperDir:gsub("/$", "") .. "/current_wallpaper.txt"
    
    local content = string.format([[Current Wallpaper Information
========================================
...
Generated by DailyWallpaper.spoon
]], 
        title or "Unknown",
        hash or "Unknown", 
        filepath or "Unknown",
        os.date("%Y-%m-%d %H:%M:%S"),
        copyright or "No description available",
        copyrightlink or "No link available"
    )
    ...
    if file then
        file:write(content)
        file:close()
        self.logger.i("Wallpaper info saved to: " .. infoFile)
    else
        self.logger.e("Failed to save wallpaper info to: " .. infoFile)
    end
end
